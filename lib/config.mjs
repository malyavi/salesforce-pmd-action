...
export function resolveConfig() {
  ...
  return {
    ...
    mode,
    sourceDirs,
    // Empty on purpose: every change under the source directories is worth
    // looking at, and PMD skips what it cannot parse. See `lib/apex.mjs`.
    extensions: listInput('extensions', []),
    configPaths,
    ruleset,
 ...
 * @param {{ sourceFiles: string[], configFiles: string[] }} changes What the diff held
...
export function planScan(config, changes) {
  ...
  if (changes.sourceFiles.length > 0) {
  ...
  return {scan: 'none', reason: 'nothing in the scanned paths changed'};
}
