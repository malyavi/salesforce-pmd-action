 ...
 * caller named and, for a caller who narrowed by extension, of one of those
 * extensions.
 *
 * The directory check is what keeps a vendored copy, a fixture or a sample tree
 * out of the scan. There is deliberately **no extension filter by default**: a
 * source directory holds far more than Apex, a change to any of it is a change
 * worth looking at, and PMD is the right judge of what it can parse — it skips
 * a file whose extension names no language it knows, and reports a file it
 * cannot read as a processing error. A filter guessed here instead is a filter
